...
contract PriceEngine is Ownable, IPriceEngine {
  ...
  constructor(address _lensHub, address _oracle, address _factory, address initialOwner) Ownable(initialOwner) {
    lensHub = _lensHub;
    oracle = _oracle;
  ...
  function getMintPrice(uint256 profileId) external view override returns (uint256) {
  ...
  function calculateSuccessIndex(uint256 profileId) internal view returns (uint256) {
    ...
    (uint256 currentIgFollowers, uint256 currentIgPosts, uint256 currentYtSubscribers, uint256 currentYtVideos) =
      IOracle(oracle).getMetrics(profileId);
    ...
    uint256 lensFollowerSI =
      prevLensFollowers[profileId] == 0 ? 1e18 : currentLensFollowers.mulDiv(1e18, prevLensFollowers[profileId]);
    uint256 lensPublicationSI = prevLensPublications[profileId] == 0
      ? 1e18
      : currentLensPublications.mulDiv(1e18, prevLensPublications[profileId]);
    uint256 igFollowerSI =
      prevIgFollowers[profileId] == 0 ? 1e18 : currentIgFollowers.mulDiv(1e18, prevIgFollowers[profileId]);
    uint256 igPostSI = prevIgPosts[profileId] == 0 ? 1e18 : currentIgPosts.mulDiv(1e18, prevIgPosts[profileId]);
    uint256 ytSubscriberSI =
      prevYtSubscribers[profileId] == 0 ? 1e18 : currentYtSubscribers.mulDiv(1e18, prevYtSubscribers[profileId]);
    uint256 ytVideoSI = prevYtVideos[profileId] == 0 ? 1e18 : currentYtVideos.mulDiv(1e18, prevYtVideos[profileId]);

    uint256 si = LENS_FOLLOWER_WEIGHT.mulDiv(lensFollowerSI, 1e18) + IG_FOLLOWER_WEIGHT.mulDiv(igFollowerSI, 1e18)
      + YT_SUBSCRIBER_WEIGHT.mulDiv(ytSubscriberSI, 1e18) + LENS_PUBLICATION_WEIGHT.mulDiv(lensPublicationSI, 1e18)
      + IG_POST_WEIGHT.mulDiv(igPostSI, 1e18) + YT_VIDEO_WEIGHT.mulDiv(ytVideoSI, 1e18);
  ...
  function updateMetrics(uint256 profileId) internal {
    ...
    (uint256 igFollowers, uint256 igPosts, uint256 ytSubscribers, uint256 ytVideos) =
      IOracle(oracle).getMetrics(profileId);
