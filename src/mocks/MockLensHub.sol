...
contract MockLensHub is ILensHub {
    ...
    function getFollowNFT(uint256 profileId) external override view returns (address) {
        return followNFTs[profileId];
    }

    function getPubCount(uint256 profileId) external override view returns (uint256) {
        return pubCounts[profileId];
    }

    function getProfile(uint256 profileId) external override view returns (address) {
